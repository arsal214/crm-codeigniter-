...
class Resource_workload extends AdminController {
	...
	public function index() {
        ...
        if ($data['type'] == 'workload') {
			...
			$data['nestedheaders'] = $this->resource_workload_model->get_nestedheaders_workload(date('Y-m-d', strtotime('-7 day', strtotime(date('Y-m-d')))), date('Y-m-d'), 'day');
			$data['columns'] = $this->resource_workload_model->get_columns_workload(date('Y-m-d', strtotime('-7 day', strtotime(date('Y-m-d')))), date('Y-m-d'), 'day');
            $data['month_columns'] = $this->resource_workload_model->get_columns_workload_by_month(date('Y-m-d', strtotime('-7 day', strtotime(date('Y-m-d')))), date('Y-m-d'));
        }elseif ($data['type'] == 'chart'){
            $list_staffids           = $this->resource_workload_model->get_staff_workload($data_fill, true);
            $standard_workload = $this->resource_workload_model->get_standard_workload_by_list_staff($list_staffids, $data_fill['from_date'], $data_fill['to_date'], 'day');
			$data['data_workload'] = $this->resource_workload_model->get_data_workload($data_fill);
	...
	public function get_data_workload() {
		...
		$nestedheaders = $this->resource_workload_model->get_nestedheaders_workload($data['from_date'], $data['to_date'], $data['workload_type']);
		$columns = $this->resource_workload_model->get_columns_workload($data['from_date'], $data['to_date'], $data['workload_type']);
		echo json_encode([
			'columns' => $columns,
	...
	public function workload_kanban()
    {
        ...
        if($task_status != ''){
            if($staffsTasksWhere != ''){
                $staffsTasksWhere .= ' AND (status IN ('.$task_status.'))'; 
            }else{
                $staffsTasksWhere  = 'status IN ('.$task_status.')';
            }
        }
    ...
    public function workload_chart(){
        ...
        $list_staffids           = $this->resource_workload_model->get_staff_workload($data_fill, true);
        $standard_workload = $this->resource_workload_model->get_standard_workload_by_list_staff($list_staffids, $from_date, $to_date, 'day');
