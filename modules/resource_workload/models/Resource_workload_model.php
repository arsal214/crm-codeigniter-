...
class Resource_workload_model extends App_Model
{
    ...
    {
        if(isset($data_fill['workload_type'])){
            switch ($data_fill['workload_type']) {
                case 'day':
                    $data = $this->get_data_workload_day($data_fill, $only_data);
                    break;
                case 'week':
                    $data = $this->get_data_workload_week($data_fill, $only_data);
                    break;
                case 'month':
                    $data = $this->get_data_workload_month($data_fill, $only_data);
                    break;
                default:
                    $data = $this->get_data_workload_day($data_fill, $only_data);
                    break;
     ...
     */
    public function get_data_workload_day($data_fill, $only_data = false)
    {
        ...
        $where_status = '';
        if(isset($data_fill['task_status'])){
            foreach ($data_fill['task_status'] as $key => $value) {
                if ($where_status == '') {
            ...
            if ($where_status != '') {
                $staffsTasksWhere .= ' and (' . $where_status . ') ';
            }
        }
        ...
        foreach ($staffsTasks_recurring as $key => $task) {
            if(in_array($task['taskid'], $staffsTasks_recurring_ids) || ($task['taskid'] != '' && $task['startdate'] == '')){
                continue;
            }
        	$node = [];
        ...
        foreach ($staffsTasks as $key => $value) {
            if($value['taskid'] != '' && $value['startdate'] == ''){
                continue;
            }

            if(strtotime($task_from_date) > strtotime($value['startdate']) && $value['startdate'] != '' && $value['startdate'] != '0000-00-00'){
                $task_from_date = $value['startdate'];
            }

            if(strtotime($task_to_date) < strtotime($value['duedate']) && $value['duedate'] != '' && $value['duedate'] != '0000-00-00'){
        ...
        $standard_workload_by_list_staff = $this->get_standard_workload_by_list_staff($list_staffids, $task_from_date, $task_to_date, $workload_type);
        ...
        foreach ($data_array as $key => $value) {
            ...
            if (!isset($data[$staffid]['capacity'])) {
                ...
                $data[$staffid]['remainCapacityEstimated'] = $data[$staffid]['capacity'];
                $data[$staffid]['remainCapacity'] = $data[$staffid]['capacity'];
                $total_capacity += $data[$staffid]['capacity'];
                $data[$staffid]['staff_department'] = $value['department_name'];
                
                $data[$staffid] = $this->create_array_data_workload($from_date, $to_date, $data[$staffid], $workload_type);
                if ($only_data == false) {
                    $data_tooltip_2[$staffid] = $this->create_array_data_tooltip_workload($data_timesheets, $value['staffid'], $from_date, $to_date, [], $workload_type);
                }
            }
                ...
                if ($number_day > 0) {
                    ...
                    if (isset($data[$staffid][date('d_m_Y', strtotime($value['startdate'])) . '_e'])) {
                        ...
                        if ($note['estimate_hour'] > 0 && $only_data == false) {
                            ...
                            $e += round($note['estimate_hour'],2);
                        }
                    }
                }
            }
   
            $data[$staffid]['remainCapacityEstimated'] =  round(($data[$staffid]['remainCapacityEstimated'] - $e), 2);
            $data[$staffid]['remainCapacity'] =  round(($data[$staffid]['remainCapacity'] - $s), 2);
     ...
     * @return     array   The data workload.
     */
    public function get_data_workload_week($data_fill, $only_data = false)
    {
        if($data_fill['to_date'] == ''){
            $from_date = date('Y-m-d');
        }else{
        ...
        foreach ($data_array as $key => $value) {
            ...
            foreach ($taskstimers as $keytaskstimers => $taskstimer) {
                if (isset($data[$staffid][date('W_o', strtotime($keytaskstimers)) . '_s'])) {

                    $data[$staffid][date('W_o', strtotime($keytaskstimers)) . '_s'] += round($taskstimer, 2);
                    if ($taskstimer > 0 && $only_data == false) {
                        $data_tooltip_2[$staffid][$value['staffid'] . '_' . date('W_o', strtotime($keytaskstimers)) . '_s'] .= $value['name'] . ': ' . round($taskstimer, 2) . ' ' . _l('hours') . "\n";
            ...
            if ($note['estimate_hour'] > 0 || $total_time != []) {
                ...
                if ($number_day > 0) {
                    ...
                    while (strtotime($f_date) <= strtotime($t_date)) {
                        ...
                        if ($estimate_hour > 0 || $total_time > 0) {
                            if (isset($date_working[$staffid][$f_date]) && $date_working[$staffid][$f_date] == '1') {
                                ...
                                if (isset($data[$staffid][date('W_o', strtotime($f_date)) . '_e'])) {
                                    $data[$staffid][date('W_o', strtotime($f_date)) . '_e'] += round($_estimate_hour, 2);
                                    if ($_estimate_hour > 0 && $only_data == false) {
                                        $data_tooltip_2[$staffid][$value['staffid'] . '_' . date('W_o', strtotime($f_date)) . '_e'] .= $value['name'] . ': ' . round($_estimate_hour, 2) . ' ' . _l('hours') . "\n";
                                        $total_estimate += $_estimate_hour;
                                        $e += round($_estimate_hour,2);
                    ...
                    if (isset($data[$staffid][date('W_o', strtotime($value['startdate'])) . '_e'])) {
                        $data[$staffid][date('W_o', strtotime($value['startdate'])) . '_e'] += round($note['estimate_hour'], 2);
                        if ($note['estimate_hour'] > 0 && $only_data == false) {
                            $data_tooltip_2[$staffid][$value['staffid'] . '_' . date('W_o', strtotime($value['startdate'])) . '_e'] .= $value['name'] . ': ' . round($note['estimate_hour'], 2) . ' ' . _l('hours') . "\n";
        ...
        foreach ($data as $val) {
            ...
            foreach ($val as $key => $value) {
                if ($only_data == false) {
                    if (!(strpos($key, '_e') === false) || !(strpos($key, '_s') === false)) {
                        $k                 = explode('_', $key);
                        if ($standard_workload[$val['staff_id']][$k[1] . '-' . $k[0]] < $value && $value > 0) {
                            $overload[$key] = 1;
                        }
                        if (isset($data_timesheets[$val['staff_id']][$k[1] . '-' . $k[0]])) {
                            $leave[$key] = 1;
                        }
                    }
                }
     ...
     * @return     array   The data workload.
     */
    public function get_data_workload_month($data_fill, $only_data = false)
    {
        if($data_fill['to_date'] == ''){
            $from_date = date('Y-m-d');
        }else{
        ...
        foreach ($data_array as $key => $value) {
            ...
            foreach ($taskstimers as $keytaskstimers => $taskstimer) {
                if (isset($data[$staffid][date('m_Y', strtotime($keytaskstimers)) . '_s'])) {

                    $data[$staffid][date('m_Y', strtotime($keytaskstimers)) . '_s'] += round($taskstimer, 2);
                    if ($taskstimer > 0 && $only_data == false) {
                        $data_tooltip_2[$staffid][$value['staffid'] . '_' . date('m_Y', strtotime($keytaskstimers)) . '_s'] .= $value['name'] . ': ' . round($taskstimer, 2) . ' ' . _l('hours') . "\n";
            ...
            if ($note['estimate_hour'] > 0 || $total_time != []) {
                ...
                if ($number_day > 0) {
                    ...
                    while (strtotime($f_date) <= strtotime($t_date)) {
                        ...
                        if ($estimate_hour > 0 || $total_time > 0) {
                            if (isset($date_working[$staffid][$f_date]) && $date_working[$staffid][$f_date] == '1') {
                                ...
                                if (isset($data[$staffid][date('m_Y', strtotime($f_date)) . '_e'])) {
                                    $data[$staffid][date('m_Y', strtotime($f_date)) . '_e'] += round($_estimate_hour, 2);
                                    if ($_estimate_hour > 0 && $only_data == false) {
                                        $data_tooltip_2[$staffid][$value['staffid'] . '_' . date('m_Y', strtotime($f_date)) . '_e'] .= $value['name'] . ': ' . round($_estimate_hour, 2) . ' ' . _l('hours') . "\n";
                                        $total_estimate += $_estimate_hour;
                                        $e += round($_estimate_hour,2);
                    ...
                    if (isset($data[$staffid][date('m_Y', strtotime($value['startdate'])) . '_e'])) {
                        $data[$staffid][date('m_Y', strtotime($value['startdate'])) . '_e'] += round($note['estimate_hour'], 2);
                        if ($note['estimate_hour'] > 0 && $only_data == false) {
                            $data_tooltip_2[$staffid][$value['staffid'] . '_' . date('m_Y', strtotime($value['startdate'])) . '_e'] .= $value['name'] . ': ' . round($note['estimate_hour'], 2) . ' ' . _l('hours') . "\n";
    ...
    public function get_data_timeline($data_fill)
    {
        ...
        if ($from_date != '' && $to_date != '') {
            $staffsTasksWhere = 'IF(duedate IS NOT NULL,((startdate <= "' . $from_date . '" and duedate >= "' . $from_date . '") or (startdate <= "' . $to_date . '" and duedate >= "' . $to_date . '") or (startdate > "' . $from_date . '" and duedate < "' . $to_date . '")), IF(datefinished IS NOT NULL,((startdate <= "' . $from_date . '" and date_format(datefinished, "%Y-%m-%d") >= "' . $from_date . '") or (startdate <= "' . $to_date . '" and date_format(datefinished, "%Y-%m-%d") >= "' . $to_date . '") or (startdate > "' . $from_date . '" and date_format(datefinished, "%Y-%m-%d") < "' . $to_date . '")),(startdate >= "' . $from_date . '" and startdate <= "' . $to_date . '")))';
        } elseif ($from_date != '') {
            $staffsTasksWhere = '(startdate >= "' . $from_date . '" or IF(duedate IS NOT NULL, duedate >= "' . $from_date . '", IF(datefinished IS NOT NULL, date_format(datefinished, "%Y-%m-%d") >= "' . $from_date . '", 1=0))';
        } elseif ($to_date != '') {
            $staffsTasksWhere = '(startdate <= "' . $to_date . '" or IF(duedate IS NOT NULL,duedate <= "' . $to_date . '",IF(datefinished IS NOT NULL, date_format(datefinished, "%Y-%m-%d") <= "' . $to_date . '", 1=0)))';
        }
     ...
     * @param      string  $workload_type 
     *
     * @return     array
     */
    public function create_array_data_workload($from_date, $to_date, $array, $workload_type)
    {
        ...
        switch ($workload_type) {
            ...
            case 'week':
                ...
                while (strtotime($from_date) <= strtotime($to_date)) {
                    $week = date('W', strtotime($from_date));
                    $year = date('o', strtotime($from_date));

                    $array[$week . '_' . $year . '_e'] = 0;
                    $array[$week . '_' . $year . '_s'] = 0;

                    $from_date = date('Y-m-d', strtotime('+1 week', strtotime($from_date)));

                    if(strtotime($from_date) > strtotime($to_date)){
                        $week_2 = date('W', strtotime($to_date));
                        $year_2 = date('o', strtotime($to_date));

                        if($week . '_' . $year != $week_2 . '_' . $year_2){
                            $array[$week_2 . '_' . $year_2 . '_e'] = 0;
                            $array[$week_2 . '_' . $year_2 . '_s'] = 0;
                        }
                    }
                }
                break;
     ...
     * @return     array
     */
    public function create_array_data_tooltip_workload($data_timesheets, $staffid, $from_date, $to_date, $array, $workload_type)
    {
        if (!$this->check_format_date($from_date)) {
            $from_date = to_sql_date($from_date);
        }
        ...
        switch ($workload_type) {
            ...
            case 'week':
                ...
                while (strtotime($from_date) <= strtotime($to_date)) {
                    ...
                    if(isset($data_timesheets[$staffid][$from_date])){
                        $array[$staffid . '_' . $week . '_' . $year . '_e'] = _l('leave').': '.$data_timesheets[$staffid][$from_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------'. "\n";
                        $array[$staffid . '_' . $week . '_' . $year . '_s'] = _l('leave').': '.$data_timesheets[$staffid][$from_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------------'. "\n";
                    }else{
                        $array[$staffid . '_' . $week . '_' . $year . '_e'] = '';
                        $array[$staffid . '_' . $week . '_' . $year . '_s'] = '';
                    }
                    ...
                    if(strtotime($from_date) > strtotime($to_date)){
                        ...
                        if(isset($data_timesheets[$staffid][$to_date])){
                            $array[$staffid . '_' . $week_2 . '_' . $year_2 . '_e'] = _l('leave').': '.$data_timesheets[$staffid][$to_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------'. "\n";
                            $array[$staffid . '_' . $week_2 . '_' . $year_2 . '_s'] = _l('leave').': '.$data_timesheets[$staffid][$to_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------------'. "\n";
                        }else{
                            $array[$staffid . '_' . $week_2 . '_' . $year_2 . '_e'] = '';
                            $array[$staffid . '_' . $week_2 . '_' . $year_2 . '_s'] = '';
                        }
                    }
                }
                break;
            case 'month':
                ...
                while (strtotime($from_date) <= strtotime($to_date)) {
                    ...
                    if(isset($data_timesheets[$staffid][$from_date])){
                        $array[$staffid . '_' . $month . '_e'] = _l('leave').': '.$data_timesheets[$staffid][$from_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------'. "\n";
                        $array[$staffid . '_' . $month . '_s'] = _l('leave').': '.$data_timesheets[$staffid][$from_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------------'. "\n";
                    }else{
                        $array[$staffid . '_' . $month . '_e'] = '';
                        $array[$staffid . '_' . $month . '_s'] = '';
                    }
                    ...
                    if(strtotime($from_date) > strtotime($to_date)){
                        ...
                        if($month != $month_2){
                            if(isset($data_timesheets[$staffid][$from_date])){
                                $array[$staffid . '_' . $month_2 . '_e'] = _l('leave').': '.$data_timesheets[$staffid][$from_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------'. "\n";
                                $array[$staffid . '_' . $month_2 . '_s'] = _l('leave').': '.$data_timesheets[$staffid][$from_date]. ' ' . _l('hours'). "\n".'-------------------------------------------------------------'. "\n";
                            }else{
                                $array[$staffid . '_' . $month_2 . '_e'] = '';
                                $array[$staffid . '_' . $month_2 . '_s'] = '';
                            }
                        }
                    }
                }
     ...
     * @param      string  $from_date  The from date format dd/mm/YYYY
     * @param      string  $to_date    To date format dd/mm/YYYY
     * @param      string  $workload_type    day or week or month
     *
     * @return     array   The nestedheaders workload.
     */
    public function get_nestedheaders_workload($from_date, $to_date, $workload_type)
    {
        if (!$this->check_format_date($from_date)) {
            $from_date = to_sql_date($from_date);
        }
        ...
        $nestedheaders    = [];
        $nestedheaders[0] = [['label' => _l('staff'), 'colspan' => 7]];
        $nestedheaders[1] = [_l('name'), '', _l('capacity'), _l('remain_capacity_estimated'), _l('remain_capacity'), _l('department'), _l('role')];
        ...
        switch ($workload_type) {
            ...
            case 'week':
                ...
                while (strtotime($from_date) <= strtotime($to_date)) {
                    ...
                    array_push($nestedheaders[0], ['label' => _l('week').' '.$week . ' - ' . $year, 'colspan' => 2]);
                    array_push($nestedheaders[1], 'E');
                    array_push($nestedheaders[1], 'S');
                    ...
                    if(strtotime($from_date) > strtotime($to_date)){
                        ...
                        if($week . ' - ' . $year != $week_2 . ' - ' . $year_2){
                            array_push($nestedheaders[0], ['label' => _l('week').' '.$week_2 . ' - ' . $year_2, 'colspan' => 2]);
                            array_push($nestedheaders[1], 'E');
                            array_push($nestedheaders[1], 'S');
                        }
                    }
            ...
            case 'month':
                while (strtotime($from_date) <= strtotime($to_date)) {
                    ...
                    array_push($nestedheaders[0], ['label' => $month, 'colspan' => 2]);
                    array_push($nestedheaders[1], 'E');
                    array_push($nestedheaders[1], 'S');
                    ...
                    if(strtotime($from_date) > strtotime($to_date)){
                        ...
                        if($month != $month_2){
                            array_push($nestedheaders[0], ['label' => $month_2, 'colspan' => 2]);
                            array_push($nestedheaders[1], 'E');
                            array_push($nestedheaders[1], 'S');
                        }
                    }
     ...
     */
    public function get_nestedheaders_timeline($from_date, $to_date, $workload_type)
    {
        if (!$this->check_format_date($from_date)) {
            $from_date = to_sql_date($from_date);
        }
     ...
     */
    public function get_columns_workload($from_date, $to_date, $workload_type)
    {
        if (!$this->check_format_date($from_date)) {
            $from_date = to_sql_date($from_date);
        }
        ...
        switch ($workload_type) {
            ...
            case 'week':
                ...
                while (strtotime($from_date) <= strtotime($to_date)) {
                    ...
                    array_push($columns, ['data' => $week . '_' . $year . '_e', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                        'pattern' => '0.00',
                    ]]);
                    array_push($columns, ['data' => $week . '_' . $year . '_s', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                        'pattern' => '0.00',
                    ]]);
                    ...
                    if(strtotime($from_date) > strtotime($to_date)){
                        ...
                        if($week . '_' . $year != $week_2 . '_' . $year_2){
                            array_push($columns, ['data' => $week_2 . '_' . $year_2 . '_e', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                                'pattern' => '0.00',
                            ]]);
                            array_push($columns, ['data' => $week_2 . '_' . $year_2 . '_s', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                                'pattern' => '0.00',
                            ]]);
                        }
                    }
            ...
            case 'month':
                ...
                while (strtotime($from_date) <= strtotime($to_date)) {
                    ...
                    array_push($columns, ['data' => $month . '_e', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                        'pattern' => '0.00',
                    ]]);
                    array_push($columns, ['data' => $month . '_s', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                        'pattern' => '0.00',
                    ]]);
                    $from_date = date('Y-m-d', strtotime('+1 month', strtotime($from_date)));
                    ...
                    if(strtotime($from_date) > strtotime($to_date)){
                        ...
                        if($month != $month_2){
                            array_push($columns, ['data' => $month_2 . '_e', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                                'pattern' => '0.00',
                            ]]);
                            array_push($columns, ['data' => $month_2 . '_s', 'type' => 'numeric', 'readOnly' => true, 'numericFormat' => [
                                'pattern' => '0.00',
                            ]]);
                        }
                    }
     ...
     * @return     array   The columns workload.
     */
    public function get_columns_workload_by_month($from_date, $to_date)
    {
        $visible = [];
        $visible[1] = get_option('staff_workload_monday_visible');
        $visible[2] = get_option('staff_workload_tuesday_visible');
     ...
     */
    public function get_standard_workload_by_list_staff($list_staff, $from_date, $to_date, $workload_type)
    {
        ...
        while (strtotime($from_date) <= strtotime($to_date)) {
            foreach ($standard_workload as $key => $value) {
                switch (date('N', strtotime($from_date))) {
                    case 1:
                        $working_days[$value['staffid']][$from_date] = is_numeric($value['monday']) ? $value['monday'] : 0;
                        break;
                    case 2:
                        $working_days[$value['staffid']][$from_date] = is_numeric($value['tuesday']) ? $value['tuesday'] : 0;
                        break;
                    case 3:
                        $working_days[$value['staffid']][$from_date] = is_numeric($value['wednesday']) ? $value['wednesday'] : 0;
                        break;
                    case 4:
                        $working_days[$value['staffid']][$from_date] = is_numeric($value['thursday']) ? $value['thursday'] : 0;
                        break;
                    case 5:
                        $working_days[$value['staffid']][$from_date] = is_numeric($value['friday']) ? $value['friday'] : 0;
                        break;
                    case 6:
                        $working_days[$value['staffid']][$from_date] = is_numeric($value['saturday']) ? $value['saturday'] : 0;
                        break;
                    case 7:
                        $working_days[$value['staffid']][$from_date] = is_numeric($value['sunday']) ? $value['sunday'] : 0;
                        break;
        ...
        if(get_option('integrated_timesheet_leave') == 1 && $this->get_status_modules_all('timesheets')){
            ...
            $timesheetsWhere = [];
            if ($from_date != '' && $to_date != '') {
                $timesheetsWhere = 'IF(end_time IS NOT NULL,(((start_time <= "' . $from_date . '") and end_time >= "' . $from_date . '") or (start_time <= "' . $to_date . '" and end_time >= "' . $to_date . '") or (start_time > "' . $from_date . '" and end_time < "' . $to_date . '")), (start_time >= "' . $from_date . '" and start_time <= "' . $to_date . '"))';
            } elseif ($from_date != '') {
        ...
        switch ($workload_type) {
            ...
            case 'week':
                while (strtotime($from_date) <= strtotime($to_date)) {
                    $week = date('o-W', strtotime($from_date));

                    foreach ($list_staff as $staffid) {
                        if(!isset($working_days[$staffid][$week])){
                            $working_days[$staffid][$week] = 0;
                        }

                        if(isset($data_timesheets[$staffid][$from_date])){
                            if($working_days[$staffid][$from_date] > $data_timesheets[$staffid][$from_date]){
                                $working_days[$staffid][$week] += $working_days[$staffid][$from_date] - $data_timesheets[$staffid][$from_date];
                            }else{
                                $working_days[$staffid][$week] += 0;
                            }
                        }else{
                            $working_days[$staffid][$week] += $working_days[$staffid][$from_date];
                        }
                    }
            ...
            case 'month':
                ...
                while (strtotime($from_date) <= strtotime($to_date)) {
                    ...
                    foreach ($list_staff as $staffid) {
                        if(!isset($working_days[$staffid][$month])){
                            $working_days[$staffid][$month] = 0;
                        }

                        if(isset($data_timesheets[$staffid][$from_date])){
                            if($working_days[$staffid][$from_date] > $data_timesheets[$staffid][$from_date]){
                                $working_days[$staffid][$month] += $working_days[$staffid][$from_date] - $data_timesheets[$staffid][$from_date];
                            }else{
                                $working_days[$staffid][$month] += 0;
                            }
                        }else{
                            $working_days[$staffid][$month] += $working_days[$staffid][$from_date];
                        }
                    }
                    $from_date = date('Y-m-d', strtotime('+1 day', strtotime($from_date)));
                }
        ...
        while (strtotime($f_date) <= strtotime($t_date)) {
            if(isset($date_working[$f_date])){
                if ($date_working[$f_date] == 1) {
                    $total += is_numeric($standard_workload[$staffid][$f_date]) ? $standard_workload[$staffid][$f_date] : 0;
                }
            }
    ...
    public function get_data_capacity($data_fill){
        ...
        if ($from_date != '' && $to_date != '') {
            ...
            $tasksWhere .= ' and (startdate >= "' . $from_date . '" or IF(duedate IS NOT NULL, duedate >= "' . $from_date . '", IF(datefinished IS NOT NULL, date_format(datefinished, "%Y-%m-%d") >= "' . $from_date . '", 1=0)))';
        } elseif ($to_date != '') {
            $tasksWhere .= ' and (startdate <= "' . $to_date . '" or IF(duedate IS NOT NULL,duedate <= "' . $to_date . '",IF(datefinished IS NOT NULL, date_format(datefinished, "%Y-%m-%d") <= "' . $to_date . '", 1=0)))';
        }
        ...
        $total_capacity = 0;
        $list_staffids           = $this->get_staff_workload($data_fill, true);
        $standard_workload_by_list_staff = $this->get_standard_workload_by_list_staff($list_staffids, $from_date, $to_date, $workload_type);
        $standard_workload = $standard_workload_by_list_staff['standard_workload'];
        $data_timesheets = $standard_workload_by_list_staff['data_timesheets'];
