...
$sTable       = $this->ci->uri->segment(4) != 'sp' ? db_prefix() . 'clients' : db_prefix() . 'school_partners';
...
foreach ($rResult as $aRow) {
    ...
    if($sTable == 'tblschool_partners'){
        ...
        $url = admin_url('clients/client/' . $aRow['userid']).'?type=sp';
    ...
    $company .= '<a href="' . admin_url('clients/client/' . $aRow['userid'] . ($isPerson && $aRow['contact_id'] ? '?group=contacts&type=sp' : '?type=sp')) . '">' . _l('view') . '</a>';

    if ($aRow['registration_confirmed'] == 0 && is_admin()) {
        $company .= ' | <a href="' . admin_url('clients/confirm_registration/' . $aRow['userid']) . '?type=sp" class="text-success bold">' . _l('confirm_registration') . '</a>';
    }
    if (!$isPerson) {
        $company .= ' | <a href="' . admin_url('clients/client/' . $aRow['userid'] . '?group=contacts&type=sp') . '">' . _l('customer_contacts') . '</a>';
    }
    if ($hasPermissionDelete) {
        $company .= ' | <a href="' . admin_url('clients/delete/' . $aRow['userid']) . '?type=sp" class="text-danger _delete">' . _l('delete') . '</a>';
    }
