...
$statuses              = $CI->school_leads_model->get_status();
$sIndexColumn = 'id';
$sTable       = db_prefix() .'leads_school';

$aColumns = [
    '1', $sTable.'.id as id', $sTable.'.name as name',
    ];
if (is_gdpr() && $consentLeads == '1') {
    $aColumns[] = '1';
}
$aColumns = array_merge($aColumns, ['company', $sTable.'.email as email', $sTable.'.phonenumber as phonenumber',
    'lead_value',
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' .$sTable.'.id and rel_type="lead" ORDER by tag_order ASC LIMIT 1) as tags',
...
$join = [
    ...
    'JOIN ' . db_prefix() . 'leads_sources ON ' . db_prefix() . 'leads_sources.id = ' .$sTable.'.source',
];
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN ' . db_prefix() . 'customfieldsvalues as ctable_' . $key . ' ON ' .$sTable.'.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}

$where  = [];
$filter = false;

if ($CI->input->post('custom_view')) {
    ...
    if ($filter == 'lost') {
        ...
        array_push($where, 'AND ' .$sTable.'.id IN (SELECT lead_id FROM ' . db_prefix() . 'consents WHERE purpose_id=' . $CI->db->escape_str(strafter($filter, 'consent_')) . ' and action="opt-in" AND date IN (SELECT MAX(date) FROM ' . db_prefix() . 'consents WHERE purpose_id=' . $CI->db->escape_str(strafter($filter, 'consent_')) . ' AND lead_id=' .$sTable.'.id))');
    }
}
...
if (has_permission('leads', '', 'view') && $CI->input->post('assigned')) {
    array_push($where, 'AND assigned =' . $CI->db->escape_str($CI->input->post('assigned')));
}

if ($CI->input->post('status')
    && count($CI->input->post('status')) > 0
    && ($filter != 'lost' && $filter != 'junk')) {
    array_push($where, 'AND status IN (' . implode(',', $CI->db->escape_str($CI->input->post('status'))) . ')');
}

if ($CI->input->post('source')) {
    array_push($where, 'AND source =' . $CI->db->escape_str($CI->input->post('source')));
}
...
$additionalColumns = hooks()->apply_filters('leads_table_additional_columns_sql', [
    ...
    'status',
    'assigned',
    'lastname as assigned_lastname', $sTable.'.addedfrom as addedfrom',
...
foreach ($rResult as $aRow) {
    ...
    $hrefAttr = 'href="' . admin_url('school_leads/index/' . $aRow['id']) .'/'. ($CI->uri->segment(4) != 'sl' ? '' : '?type=sl'). '" onclick="init_school_lead(' . $aRow['id'] . ');return false;"';
    ...
    if (!$locked) {
        $nameRow .= ' | <a href="' . admin_url('school_leads/index/' . $aRow['id'] . '?edit=true') .'/'. (($CI->uri->segment(4) == 'sl') ? '?type=sl' : ''). '" onclick="init_school_lead(' . $aRow['id'] . ', true);return false;">' . _l('edit') . '</a>';
    }

    if ($aRow['addedfrom'] == get_staff_user_id() || $has_permission_delete) {
        $nameRow .= ' | <a href="' . admin_url('school_leads/delete/' . $aRow['id']) .'/'. (($CI->uri->segment(4) == 'sl') ? '?type=sl' : ''). '" class="_delete text-danger">' . _l('delete') . '</a>';
    }
    $nameRow .= '</div>';
    ...
    if (is_gdpr() && $consentLeads == '1') {
        ...
        $consents    = $CI->gdpr_model->get_consent_purposes($aRow['id'], 'lead');
