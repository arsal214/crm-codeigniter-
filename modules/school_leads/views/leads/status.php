...
<div class="modal fade" id="status" tabindex="-1" role="dialog">
   <div class="modal-dialog">
      ...
      $status_count = 0;
      if($url == "statuses_sl"){
           $status_count =  $this->db->get_where(db_prefix() . 'leads_status',["type" => "sl"])->num_rows();
      }else{
          $status_count =  $this->db->get(db_prefix() . 'leads_status')->num_rows();
      }
      echo form_open(admin_url('school_leads/status'), ['id' => 'leads-status-form']); ?>
         ...
         <div class="modal-body">
            <div class="row">
               <div class="col-md-12">
                  ...
                  <?php echo render_input('statusorder', 'leads_status_add_edit_order', ($status_count+1), 'number'); ?>
               </div>
            </div>
         </div>
...
function manage_leads_statuses(form) {
    if(last_part == "statuses_sl"){
        var url = form.action+"?type=sl";
    }else{
        var url = form.action;
    }
