...
class School_leads_model extends App_Model
{
	...
	*/	
    ...
    {
        $this->db->select('*,' . db_prefix() . 'leads_school.name, ' . db_prefix() . 'leads_school.id,' . db_prefix() . 'school_leads_status.name as status_name,' . db_prefix() . 'school_leads_sources.name as source_name');
        $this->db->join(db_prefix() . 'school_leads_status', db_prefix() . 'school_leads_status.id=' . db_prefix() . 'leads_school.status', 'left');
        $this->db->join(db_prefix() . 'school_leads_sources', db_prefix() . 'school_leads_sources.id=' . db_prefix() . 'leads_school.source', 'left');

        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'leads_school.id', $id);
            $lead = $this->db->get(db_prefix() . 'leads_school')->row();
            if ($lead) {
        ...
        return $this->db->get(db_prefix() . 'leads_school')->result_array();
    }
     ...
     */
    public function get_school_leads($id = '', $where = [])
    {
        $this->db->select('*,' . db_prefix() . 'leads_school.name, ' . db_prefix() . 'leads_school.id,' . db_prefix() . 'school_leads_status.name as status_name,' . db_prefix() . 'school_leads_sources.name as source_name');
        $this->db->join(db_prefix() . 'school_leads_status', db_prefix() . 'school_leads_status.id=' . db_prefix() . 'leads_school.status', 'left');
        $this->db->join(db_prefix() . 'school_leads_sources', db_prefix() . 'school_leads_sources.id=' . db_prefix() . 'leads_school.source', 'left');
    ...
    public function add($data)
    {
        ...
        if(isset($data['school_table'])){
            unset($data['school_table']);
            $this->db->insert(db_prefix() . 'leads_school', $data);
        }else{
            $this->db->insert(db_prefix() . 'leads_school', $data);
        }
    ...
    public function lead_assigned_member_notification($lead_id, $assigned, $integration = false)
    {
        if ((!empty($assigned) && $assigned != 0)) {
            ...
            $name = $this->db->select('name')->from(db_prefix() . 'leads_school')->where('id', $lead_id)->get()->row()->name;
            ...
            $this->db->update(db_prefix() . 'leads_school', [
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'leads_school', $data);
    ...
    public function update_get_school_leads($data, $id)
    {
        unset($data['school_table']);
        $current_lead_data = $this->get_school_leads($id);
        $current_status    = $this->get_status($current_lead_data->status);
        if ($current_status) {
            $current_status_id = $current_status->id;
            $current_status    = $current_status->name;
    ...
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'leads_school');
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('fieldto', 'leads_school');
    ...
    public function delete_school_leads($id)
    {
        $affectedRows = 0;
    ...
    public function mark_as_lost($id)
    {
        ...
        $this->db->from(db_prefix() . 'leads_school');
        $this->db->where('id', $id);
    ...
    {
        if($this->uri->segment(3) == "school_sources" || isset($_GET["type"])){
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix() . 'school_leads_sources')->row();
        }

        $this->db->order_by('name', 'asc');

        return $this->db->get(db_prefix() . 'school_leads_sources')->result_array();
    }
    ...
    {
        $this->db->insert(db_prefix() . 'school_leads_sources', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'school_leads_sources', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_source($id)
    {
        ...
        if (is_reference_in_table('source', db_prefix() . 'leads_school', $id) || is_reference_in_table('lead_source', db_prefix() . 'leads_email_integration', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'school_leads_sources');
        if ($this->db->affected_rows() > 0) {
    ...
    public function get_status($id = '', $where = [])
    {
        ...
        $this->db->where($where);
        if($this->uri->segment(3) == "statuses_sl" || isset($_GET["type"])){
                $this->db->where("type","sl");
        }else{
            $this->db->where("type",null);
        }
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix() . 'school_leads_status')->row();
        }
        ...
        if (!$statuses) {
            ...
            $statuses = $this->db->get(db_prefix() . 'school_leads_status')->result_array();
    ...
    public function add_status($data)
    {
        ...
        if (!isset($data['statusorder'])) {
            $data['statusorder'] = total_rows(db_prefix() . 'school_leads_status') + 1;
        }

        $this->db->insert(db_prefix() . 'school_leads_status', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'school_leads_status', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_status($id)
    {
        ...
        // Check if is already using in table
        if (is_reference_in_table('status', db_prefix() . 'leads_school', $id) || is_reference_in_table('lead_status', db_prefix() . 'leads_email_integration', $id)) {
            return [
                'referenced' => true,
            ];
        }
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'school_leads_status');
        if ($this->db->affected_rows() > 0) {
    ...
    public function update_lead_status($data)
    {
        ...
        if(isset($_GET["type"])){
            $_old = $this->db->get(db_prefix() . 'leads_school')->row();
        }else{
            $_old = $this->db->get(db_prefix() . 'leads_school')->row();
        }
        ...
        if (isset($data['order'])) {
            if(isset($_GET["type"]))
                AbstractKanban::updateOrder($data['order'], 'leadorder', 'leads_school', $data['status']);
            else
                AbstractKanban::updateOrder($data['order'], 'leadorder', 'leads_school', $data['status']);
        }
    ...
    public function staff_can_access_lead($id, $staff_id = '')
    {
        ...
        if (has_permission('leads_school', $staff_id, 'view')) {
            return true;
        }

        $CI = &get_instance();

        if (total_rows(db_prefix() . 'leads_school', 'id="' . $CI->db->escape_str($id) . '" AND (assigned=' . $CI->db->escape_str($staff_id) . ' OR is_public=1 OR addedfrom=' . $CI->db->escape_str($staff_id) . ')') > 0) {
            return true;
        }
    ...
    public function change_status_color($data)
    {
        ...
        $this->db->update(db_prefix() . 'school_leads_status', [
            'color' => $data['color'],
        ]);
    }
