...
class School_leads extends AdminController 
{
	public function __construct()
    {
        parent::__construct();		
        // $this->load->model('school_leads');
        $this->load->model("school_leads_model");
        generate_table();
    ...
    public function index(){
        ...
        $data['summary']  = get_leads_school_summary();
        $data['statuses'] = $this->school_leads_model->get_status();
        $data['sources']  = $this->school_leads_model->get_source();
        $data['title']    = "School Leads";
    ...
    public function table()
    {
        ...
        get_table_school_leads_data('leads');
    }
    ...
    public function lead($id = '')
    {
        ...
        if (!is_staff_member() || ($id != '' && !$this->school_leads_model->staff_can_access_lead($id))) {
            ajax_access_denied();
        }
        if ($this->input->post()) {
            if ($id == '') {
                if(isset($_POST['school_table']) || isset($_GET['type'])){

                    $check_table = 'school_lead';
                }else{
                    $check_table = '';
                }

                $id      = $this->school_leads_model->add($this->input->post());
                $message = $id ? _l('added_successfully', _l('lead')) : '';
                echo json_encode([
                    'success'  => $id ? true : false,
                    'id'       => $id,
                    'message'  => $message,
                    'leadView' => $id ? $this->_get_lead_data($id,$check_table) : [],
                ]);
                
            } else {
                close_setup_menu();

                if(isset($_GET['school_table']) || $_POST['school_table'] || isset($_GET['type'])){

                    $emailOriginal   = $this->db->select('email')->where('id', $id)->get(db_prefix() . 'leads_school')->row()->email;
                    $proposalWarning = false;
                    $message         = '';
                    $success         = $this->school_leads_model->update_get_school_leads($this->input->post(), $id);
                    if ($success) {
                        $emailNow = $this->db->select('email')->where('id', $id)->get(db_prefix() . 'leads_school')->row()->email;

                        $proposalWarning = total_rows(db_prefix() . 'proposals', [
                                'rel_type' => 'lead',
                                'rel_id'   => $id, ]) > 0 && ($emailOriginal != $emailNow) && $emailNow != '';
                    ...
                    echo json_encode([
                        ...
                        'leadView'         => $this->_get_lead_data($id,'school_lead'),
                    ]);
                }else{
                    $emailOriginal   = $this->db->select('email')->where('id', $id)->get(db_prefix() . 'leads')->row()->email;
                    $proposalWarning = false;
                    $message         = '';
                    $success         = $this->school_leads_model->update($this->input->post(), $id);
                    if ($success) {
    ...
    private function _get_lead_data($id = '',$table = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            if($table == 'school_lead' || isset($_GET['school_table']) || isset($_GET['type'])){
                $lead = $this->school_leads_model->get_school_leads($id, $leadWhere);
                $data['action_url'] = 'school_lead';
            }else{
                $lead = $this->school_leads_model->get($id, $leadWhere);
            }
            if (!$lead) {
            ...
            $data['lead']          = $lead;
            $data['mail_activity'] = $this->school_leads_model->get_mail_activity($id);
            $data['notes']         = $this->misc_model->get_notes($id, 'lead');
            $data['activity_log']  = $this->school_leads_model->get_lead_activity_log($id);
    ...
    public function leads_kanban_load_more()
    {
        ...
        if(isset($_GET['type'])){
            $leads = (new SchoolLeadsKanban($status['id']))
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('school_leads/sl'));
        }
        ...
        if(isset($_GET['type'])){
            $response = $this->school_leads_model->delete_school_leads($id);
        }else{
            $response = $this->school_leads_model->delete($id);
        }
        ...
        if (!$ref || strpos($ref, 'index/' . $id) !== false) {
            redirect(admin_url('school_leads'));
        }
    ...
    {
        if (!is_staff_member() || !$this->school_leads_model->staff_can_access_lead($id)) {
            ajax_access_denied();
        }
        $message = '';
        $success = $this->school_leads_model->mark_as_lost($id);
        if ($success) {
    ...
    public function unmark_as_lost($id)
    {
        ...
        $message = '';
        $success = $this->school_leads_model->unmark_as_lost($id);
        if ($success) {
    ...
    public function mark_as_junk($id)
    {
        ...
        $message = '';
        $success = $this->school_leads_model->mark_as_junk($id);
        if ($success) {
    ...
    public function unmark_as_junk($id)
    {
        ...
        $message = '';
        $success = $this->school_leads_model->unmark_as_junk($id);
        if ($success) {
    ...
    public function add_activity()
    {
        ...
        if (!is_staff_member() || !$this->school_leads_model->staff_can_access_lead($leadid)) {
            ajax_access_denied();
        }
        if ($this->input->post()) {
            $message = $this->input->post('activity');
            $aId     = $this->school_leads_model->log_lead_activity($leadid, $message);
            if ($aId) {
    ...
    public function get_convert_data($id)
    {
        ...
        $data['lead'] = $this->school_leads_model->get($id);
    ...
    public function convert_to_customer()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id) {
                ...
                $this->school_leads_model->log_lead_activity($data['leadid'], 'not_lead_activity_converted', false, serialize([
                    get_staff_full_name(),
                ]));
                $default_status = $this->school_leads_model->get_status('', [
                    'isdefault' => 1,
                ]);
                ...
                if ($contact->email != $original_lead_email) {
                    if ($original_lead_email != '') {
                        $this->school_leads_model->log_lead_activity($data['leadid'], 'not_lead_activity_converted_email', false, serialize([
                            $original_lead_email,
                            $contact->email,
                        ]));
                    }
                ...
                if (is_gdpr() && get_option('gdpr_after_lead_converted_delete') == '1') {
                    ...
                    $this->school_leads_model->delete($data['leadid']);
    ...
    public function update_lead_status()
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            $this->school_leads_model->update_lead_status($this->input->post());
        }
    }
    ...
    public function update_status_order()
    {
        if ($post_data = $this->input->post()) {
            $this->school_leads_model->update_status_order($post_data);
        }
    }
    ...
    {
        if ($this->input->post()) {
            $this->school_leads_model->add_attachment_to_database(
    ...
    {
        if (!is_staff_member() || !$this->school_leads_model->staff_can_access_lead($lead_id)) {
            ajax_access_denied();
        }
        echo json_encode([
            'success'  => $this->school_leads_model->delete_lead_attachment($id),
    ...
    public function form($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($id == '') {
                $data = $this->input->post();
                $id   = $this->school_leads_model->add_form($data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('web_to_lead_form')));
                    redirect(admin_url('leads/form/' . $id));
                }
            } else {
                $success = $this->school_leads_model->update_form($id, $this->input->post());
                if ($success) {
        ...
        if ($id != '') {
            $data['form'] = $this->school_leads_model->get_form([
                'id' => $id,
            ]);
    ...
    public function delete_form($id)
    {
        ...
        $success = $this->school_leads_model->delete_form($id);
        if ($success) {
    ...
    public function source()
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                ...
                $id = $this->school_leads_model->add_source($data);
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->school_leads_model->update_source($data, $id);
                if ($success) {
    ...
    public function delete_source($id)
    {
        ...
        $response = $this->school_leads_model->delete_source($id);
        if (is_array($response) && isset($response['referenced'])) {
            ...
            redirect(admin_url('school_leads/sources'));
    }
    ...
    public function status()
    {
        ...
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                ...
                $id = $this->school_leads_model->add_status($data);
                if (!$inline) {
                    if ($id) {
                ...
                $id = $data['id'];
                unset($data['id']);
                $success = $this->school_leads_model->update_status($data, $id);
                if ($success) {
    ...
    public function delete_status($id)
    {
        ...
        $response = $this->school_leads_model->delete_status($id);
        if (is_array($response) && isset($response['referenced'])) {
        ...
        if(isset($_GET["type"])){
            redirect(admin_url('leads/statuses_sl'));
        }else{
            redirect(admin_url('leads/statuses'));
        }
    }
    ...
    {
        if (!is_staff_member() || !$this->school_leads_model->staff_can_access_lead($rel_id)) {
            ajax_access_denied();
        }
        ...
        if ($this->input->post()) {
            ...
            if ($note_id) {
                if (isset($contacted_date)) {
                    ...
                    if ($this->db->affected_rows() > 0) {
                        $this->school_leads_model->log_lead_activity($rel_id, 'not_lead_activity_contacted', false, serialize([
    ...
    public function test_email_integration()
    {
        ...
        $mail     = $this->school_leads_model->get_email_integration();
    ...
    public function email_integration()
    {
        ...
        if ($this->input->post()) {
            ...
            $success = $this->school_leads_model->update_email_integration($data);
            if ($success) {
        ...
        $data['mail']  = $this->school_leads_model->get_email_integration();
    ...
    public function change_status_color()
    {
        if ($this->input->post() && is_admin()) {
            $this->school_leads_model->change_status_color($this->input->post());
        }
    }
                ...
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        if ($has_permission_delete) {
                            if($_POST['type'] == 'sl'){
                                if ($this->school_leads_model->delete_school_leads($id)) {
                                    $total_deleted++;
                                }
                            }else{
                                if ($this->school_leads_model->delete($id)) {
                                    $total_deleted++;
                                }
                            }
                        }
                        ...
                        if ($status || $source || $assigned || $last_contact || $visibility) {
                            ...
                            if ($status) {
                                ...
                                $this->school_leads_model->update_lead_status([
                                    'status' => $status,
                                    'leadid' => $id,
                                ]);
                            }
                        ...
                        if ($lost == 'true') {
                            $this->school_leads_model->mark_as_lost($id);
                        }
                    }
                }
            }
    ...
    public function download_files($lead_id)
    {
        ...
        $files = $this->school_leads_model->get_lead_attachments($lead_id);
