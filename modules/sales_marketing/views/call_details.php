...
if (!empty($details->module_field_id) && !empty($details->module) && $details->module == 'sam') {
    $client_info = $this->sam_model->get_deal_customers($details->module_field_id);
    $client_info['company'] = $client_info['name'];
    // array to object
    $client_info = (object)$client_info;
}
$user = $this->sam_model->check_deals_by(array('staffid' => $details->user_id), db_prefix() . 'staff');
?>
