...
<div class="panel_s no-padding mbot5 mergeable-invoices">
    ...
    <div class="panel-body">
        ...
        <div class="tw-flex tw-justify-between tw-items-center tw-mb-2 last:tw-mb-0">
            <div>
                <a href="<?php echo admin_url(SAM_MODULE.'/invoices/#' . $_inv->id); ?>" target="_blank"
        ...
        <div class="panel-body">
                ...
                <div class="horizontal-tabs">
                    <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
                        ...
                        <li role="presentation" class="tab-separator">
                            <a href="#tab_notes"
                                onclick="get_sales_notes(<?php echo e($invoice->id); ?>,'<?=SAM_MODULE?>/invoices'); return false"
                ...
                <div class="col-md-9 _buttons">
                    ...
                    <div class="pull-right">
                     ...
                     ?>
                        ...
                        <a href="<?php echo admin_url(SAM_MODULE.'/invoices/invoice/' . $invoice->id .'/'. $invoice->sam_id); ?>" data-toggle="tooltip"
                         ...
                         echo ' PDF';
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li class="hidden-xs"><a
                                        href="<?php echo admin_url(SAM_MODULE.'/invoices/pdf/' . $invoice->id . '?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a>
                                </li>
                                <li class="hidden-xs"><a
                                        href="<?php echo admin_url(SAM_MODULE.'/invoices/pdf/' . $invoice->id . '?output_type=I'); ?>"
                                        target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
                                <li><a
                                        href="<?php echo admin_url(SAM_MODULE.'/invoices/pdf/' . $invoice->id); ?>"><?php echo _l('download'); ?></a>
                                </li>
                                <li>
                                    <a href="<?php echo admin_url(SAM_MODULE.'/invoices/pdf/' . $invoice->id . '?print=true'); ?>"
                                        target="_blank">
                                        <?php echo _l('print'); ?>
                                    </a>
                                </li>
                         ...
                         echo ' disabled';
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <li>
                                        ...
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/invoices/send_overdue_notice/' . $invoice->id); ?>">
                                            <?php echo _l('send_overdue_notice_tooltip'); ?>
                                        </a>
                                        <?php } ?>
                                    </li>
                                    <?php 
                                    if ($invoice->status != Invoices_model::STATUS_CANCELLED 
                                        && staff_can('create',  'credit_notes')
                                        && !empty($invoice->clientid) && 0) {?>
                                    <li>
                                        <a href="<?php echo admin_url(SAM_MODULE.'/credit_notes/credit_note_from_invoice/' . $invoice->id); ?>"
                                    ...
                                    <li>
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/invoices/copy/' . $invoice->id .'/'. $invoice->sam_id); ?>"><?php echo _l('invoice_copy'); ?></a>
                                    </li>
                                    <?php } ?>
                                    <?php if ($invoice->sent == 0) { ?>
                                    <li>
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/invoices/mark_as_sent/' . $invoice->id); ?>"><?php echo _l('invoice_mark_as_sent'); ?></a>
                                    </li>
                                    <?php } ?>
                              ...
                              && $invoice->status != Invoices_model::STATUS_PARTIALLY) { ?>
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/invoices/mark_as_cancelled/' . $invoice->id); ?>"><?php echo e(_l('invoice_mark_as', _l('invoice_status_cancelled'))); ?></a>
                                        <?php } elseif ($invoice->status == Invoices_model::STATUS_CANCELLED) { ?>
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/invoices/unmark_as_cancelled/' . $invoice->id); ?>"><?php echo e(_l('invoice_unmark_as', _l('invoice_status_cancelled'))); ?></a>
                                        <?php } ?>
                                    </li>
                                    <?php } ?>
                           ...
                           && is_invoices_overdue_reminders_enabled()) { ?>
                                    <li>
                                        ...
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/invoices/resume_overdue_reminders/' . $invoice->id); ?>"><?php echo _l('resume_overdue_reminders'); ?></a>
                                        <?php } else { ?>
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/invoices/pause_overdue_reminders/' . $invoice->id); ?>"><?php echo _l('pause_overdue_reminders'); ?></a>
                                        <?php } ?>
                                    </li>
                                    <?php } ?>
                                    <?php
                           if ((get_option('delete_only_on_last_invoice') == 1 && is_last_invoice($invoice->id)) || (get_option('delete_only_on_last_invoice') == 0)) { ?>
                                    ...
                                    <li data-toggle="tooltip" data-title="<?php echo _l('delete_invoice_tooltip'); ?>">
                                        <a href="<?php echo admin_url(SAM_MODULE.'/invoices/delete/' . $invoice->id .'/'. $invoice->sam_id); ?>"
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane active" id="tab_invoice">
                    ...
                    <?php } ?>
                    <?php $this->load->view(SAM_MODULE.'/invoices/invoice_preview_html'); ?>
                </div>
                <?php if (count($invoice->payments) > 0) { ?>
                <div class="tab-pane" role="tabpanel" id="invoice_payments_received">
                    <?php 
                    //include_once(APPPATH . 'views/'.SAM_MODULE.'/invoices/invoice_payments_table.php'); 
                    $this->load->view(SAM_MODULE.'/invoices/invoice_payments_table');
                    ?>
                </div>
                <?php } ?>
                <?php if (count($applied_credits) > 0) { ?>
                        ...
                        <table class="table table-bordered table-hover no-mtop">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/credit_notes/list_credit_notes/' . $credit['credit_id']); ?>"><?php echo e(format_credit_note_number($credit['credit_id'])); ?></a>
                                    </td>
                                    ...
                                    <td><?php echo app_format_money($credit['amount'], $invoice->currency_name) ?>
                                        ...
                                        <a href="<?php echo admin_url(SAM_MODULE.'/credit_notes/delete_invoice_applied_credit/' . $credit['id'] . '/' . $credit['credit_id'] . '/' . $invoice->id); ?>"
                ...
                <div role="tabpanel" class="tab-pane" id="tab_child_invoices">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item">
                            <a href="<?php echo admin_url(SAM_MODULE.'/invoices/list_invoices/' . $recurring->id); ?>"
                ...
                <div role="tabpanel" class="tab-pane" id="tab_notes">
                    <?php echo form_open(admin_url(SAM_MODULE.'/invoices/add_note/' . $invoice->id), ['id' => 'sales-notes', 'class' => 'invoice-notes-form']); ?>
...
<?php $this->load->view(SAM_MODULE.'/invoices/invoice_send_to_client'); ?>
<?php $this->load->view(SAM_MODULE.'/credit_notes/apply_invoice_credits'); ?>
<?php $this->load->view(SAM_MODULE.'/credit_notes/invoice_create_credit_note_confirm'); ?>
<script>
...
function record_payment(e) {
    void 0 !== e && "" !== e && $("#invoice").load(admin_url + "<?=SAM_MODULE?>/invoices/record_invoice_payment_ajax/" + e)
}
</script>
