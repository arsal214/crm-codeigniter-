    ...
    <div class="alert alert-info">
        <div class="row">
            <div id="merge" class="col-md-6">
                <?php
              if (isset($invoice)) {
                  $this->load->view(SAM_MODULE.'/invoices/merge_invoice', ['invoices_to_merge' => $invoices_to_merge]);
              }
            ?>
            </div>
            ...
            <div id="expenses_to_bill" class="col-md-6">
                ...
                $this->load->view(SAM_MODULE.'/invoices/bill_expenses', ['expenses_to_bill' => $expenses_to_bill]);
            } ?>
            </div>
            <?php } ?>
        </div>
...
<div class="panel_s invoice accounting-template"> 
    ...
    echo form_hidden('merge_current_invoice', $invoice->id);
        ...
        <?php
        $sam_id=""; $contract_id=""; $proposal_id="";
        if(isset($invoice)){
            if(isset($invoice->sam_id)){
                $sam_id = $invoice->sam_id;
            }
            if(isset($invoice->contract_id)){
                $contract_id = $invoice->contract_id;
            }
            if(isset($invoice->pro_id)){
                $proposal_id = $invoice->pro_id;
            }    
        }
        else{
            if(isset($proposal->sam_id)){
                $sam_id = $proposal->sam_id;
            }
            if(isset($proposal->contract_id)){
        ...
        <input type="hidden" name="pro_id" value="<?=$proposal_id?>">
        <div class="row">
            <div class="col-md-6">
                ...
                <!--<div class="form-group">
                    ...
                    <div class="dropdown" style="width: 100%;">
                        <select id="pro_id" name="pro_id" class="selectpicker" data-width="100%" class="" readonly>
                            <?php
                            if($proposal_id!=""){                                           
                                //echo '<option value="'.$proposal_id.'"  selected="selected">'.e(format_proposal_number($proposal_id)).'</option>';
                            }
                            ?>
                        </select>
                    </div>
                ...
                <!--<div class="form-group">
                    <label for="contract_id" class="control-label">
                        <span class="text-danger">*</span>
                        Contract
                    </label>
                    <div class="dropdown" style="width: 100%;">
                        <select id="contract_id" name="contract_id" class="selectpicker" data-width="100%" class="" readonly>
                            <?php
                            if(isset($invoice->contract_id)){                                           
                                //echo '<option value="'.$proposal->contract_id.'"  selected="selected">'.$proposal->contract_id.'</option>';
                            }
                            ?>
                        </select>
                    </div>
                ...
                <div class="f_client_id">
                    <div class="form-group select-placeholder">
                        ...
                        <select id="clientid" name="clientid" data-live-search="" data-width="100%" class="selectpicker" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php
                        //include_once(APPPATH . 'views/admin/invoices/billing_and_shipping_template.php');
                        $this->load->view(SAM_MODULE."/invoices/billing_and_shipping_template.php"); 
                        ?>
                    </div>
                    <div class="col-md-6">
               ...
               $next_invoice_number = (int)$next_invoice_number;  // Casting to integer
               ...
               $padding_length = get_option('number_padding_prefixes');
               $_invoice_number = str_pad($__number, $padding_length, '0', STR_PAD_LEFT);
            ...
            <div class="col-md-6">
                <div class="tw-ml-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                     <?php
                        // Ensure $staff array has the correct structure
                        foreach ($staff as &$member) {
                            $member['full_name'] = $member['firstname'] . ' ' . $member['lastname'];
                        }
                        
                        // Determine the selected staff ID
                        $selected = '';
                        if (isset($invoice) && !empty($invoice->sale_agent)) {
                            $selected = $invoice->sale_agent;
                        } elseif (get_option('automatically_set_logged_in_staff_sales_agent') == '1') {
                            $selected = get_staff_user_id();
                        }
                        
                        // Debugging: Uncomment to check the selected value
                        // echo "Selected Sale Agent ID: " . $selected; exit;
                        
                        // Render the select dropdown
                        echo render_select('sale_agent', $staff, ['staffid', 'full_name'], 'sale_agent_string', $selected);
                        ?>
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-md-4">
                <?php $this->load->view(SAM_MODULE.'/invoice_items/item_select'); ?>
            </div>
