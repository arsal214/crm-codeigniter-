...
$proposal_id = "";
if (isset($contract->pro_id) && $contract->pro_id != null) {
    $proposal_id = $contract->pro_id;    
}
?>
<div id="wrapper">
    <div class="content">
        <div class="row">
            <?php
            if (isset($contract) && ($contract->signed == 1 || $contract->marked_as_signed == 1)) { ?>
            <div class="col-md-12">
                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <div class="form-group select-placeholder f_client_id">
                            ...
                            <select id="clientid" name="client" data-live-search="false" data-width="100%"
                                class="selectpicker"
                ...
                <div class="sm:tw-flex sm:tw-justify-between sm:tw-items-center tw-mb-1 -tw-mt-px">
                    ...
                    <div>
                        <div class="_buttons tw-space-x-1 tw-flex tw-items-center">
                            <a href="<?php echo admin_url(SAM_MODULE.'/proposals/#' . $contract->pro_id); ?>"
                                target="_blank">
                                <?php echo e(format_proposal_number($contract->pro_id)); ?>
                            </a>
                            | 
                            ...
                            <div class="btn-group !tw-ml-3">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li class="hidden-xs"><a
                                            href="<?php echo admin_url(SAM_MODULE.'/contracts/pdf/' . $contract->id .'/'.$sam_id. '?output_type=I'); ?>">
                                            <?php echo _l('view_pdf'); ?>
                                        </a>
                                    </li>
                                    <li class="hidden-xs">
                                        <a href="<?php echo admin_url(SAM_MODULE.'/contracts/pdf/' . $contract->id .'/'.$sam_id. '?output_type=I'); ?>"
                                            target="_blank"><?php echo _l('view_pdf_in_new_window'); ?>
                                        </a>
                                    </li>
                                    <li><a
                                            href="<?php echo admin_url(SAM_MODULE.'/contracts/pdf/' . $contract->id.'/'.$sam_id); ?>"><?php echo _l('download'); ?></a>
                                    </li>
                                    <li>
                                        <a href="<?php echo admin_url(SAM_MODULE.'/contracts/pdf/' . $contract->id .'/'.$sam_id. '?print=true'); ?>"
                                            target="_blank">
                                            <?php echo _l('print'); ?>
                                        </a>
                                    </li>
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <li>
                                        <a href="<?php echo admin_url(SAM_MODULE.'/contracts/mark_as_signed/' . $contract->id. '/'. $sam_id); ?>">
                                    ...
                                    <li>
                                        <a
                                            href="<?php echo admin_url(SAM_MODULE.'/contracts/unmark_as_signed/' . $contract->id. '/'. $sam_id); ?>">
                                    ...
                                    <li>
                                        <a href="<?php echo admin_url(SAM_MODULE.'/contracts/copy/' . $contract->id. '/'. $sam_id); ?>">
                                    ...
                                    <li>
                                        <a href="<?php echo admin_url(SAM_MODULE.'/contracts/clear_signature/' . $contract->id. '/'. $sam_id); ?>"
                                            class="_delete">
                                    ...
                                    <li>
                                        <a href="<?php echo admin_url(SAM_MODULE.'/contracts/delete/' . $contract->id. '/'.$sam_id); ?>"
                                            class="_delete">
                                            <?php echo _l('delete'); ?>
                                        </a>
                                    </li>
                            ...
                            <?php 
                                if (staff_can('create',  'invoices')) { ?>
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-success dropdown-toggle<?php if (!$isSignedOrMarkedSigned) {
                                                    echo ' disabled';
                                                } ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        ...
                                        <?php
                                         ...
                                         } ?>><?php echo "Invoice";//_l('proposal_convert_estimate'); ?></a></li>
                                            <?php } ?> 
                                        </ul>
                                    </div>
                                <?php 
                                ...
                                {
                                     //if ($contract->invoice_id != null) {
                                     if (0) {
                                         //echo '<a href="' . admin_url('estimates/list_invoices/' . $contract->invoice_id) . '" class="btn btn-primary">' . e(format_estimate_number($contract->estimate_id)) . '</a>';
                                     } else {
                                         //echo '<a href="' . admin_url('invoices/list_invoices/' . $contract->invoice_id) . '" class="btn btn-primary">' . e(format_invoice_number($contract->invoice_id)) . '</a>';
                                     }
                                } ?>
                                    ...
                                    <div class="display-block">
                                        <div class="media-body">
                                            <div class="display-block">
                                                ...
                                                <a href="<?php echo admin_url(SAM_MODULE.'/contracts/delete_renewal/' . $renewal['id'] . '/' . $renewal['contractid'] .'/'. $sam_id); ?>"
...
<?php $this->load->view(SAM_MODULE.'/contracts/send_to_client'); ?>
<?php $this->load->view(SAM_MODULE.'/contracts/renew_contract'); ?>
<?php } ?>
...
function add_contract_comment() {
    ...
    $.post(admin_url + '<?=SAM_MODULE?>/contracts/add_comment', data).done(function(response) {
        response = JSON.parse(response);
...
function get_contract_comments() {
    ...
    requestGet('<?=SAM_MODULE?>/contracts/get_comments/' + contract_id).done(function(response) {
...
function remove_contract_comment(commentid) {
    if (confirm_delete()) {
        requestGetJSON('<?=SAM_MODULE?>/contracts/remove_comment/' + commentid).done(function(response) {
            if (response.success == true) {
...
function edit_contract_comment(id) {
    ...
    if (content != '') {
        $.post(admin_url + '<?=SAM_MODULE?>/contracts/edit_comment/' + id, {
            content: content
...
function add_template(e, t) {
    $("#modal-wrapper").load(admin_url + "<?=SAM_MODULE?>/templates/modal", {
        slug: "new",
        rel_type: e,
        rel_id: t,
        sam_id: <?=$sam_id?>
...
function delete_template(e, t, a) {
    confirm_delete() && $.post(admin_url + "<?=SAM_MODULE?>/templates/delete/" + a).done((function(a) {
...
function get_templates(e, t) {
    "proposals" === e ? $("#proposal-templates").load(admin_url + "<?=SAM_MODULE?>/templates", {
        rel_type: e,
        rel_id: t,
        sam_id: <?=$sam_id?>
    }, (function() {
    ...
    )) : "contracts" === e && $("#contract-templates").load(admin_url + "<?=SAM_MODULE?>/templates", {
        rel_type: e,
        rel_id: t,
        sam_id: <?=$sam_id?>
    }, (function() {
...
function proposal_convert_template(e) {
    ...
    requestGet("<?=SAM_MODULE?>/contracts/get_" + t + "_convert_data/<?=$proposal_id.'/'.$sam_id?>").done((function(e) {
