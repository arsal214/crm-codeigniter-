...
<div class="form-group">
    <div class="input-group">
        ...
        <div class="input-group-addon" title="<?= _l('add') . ' ' . _l('manual') ?>" data-toggle="tooltip"
            ...
            <a href="<?= base_url('admin/'.SAM_MODULE.'/manuallyItems/' . $id) ?>"
...
<div id="dealItems">
    <?php $this->load->view(SAM_MODULE.'/deals_details/dealItems') ?>
</div>
...
<script type="text/javascript">
    ...
    $("#" + forItem).autocomplete({
        source: function (request, response) {
            $.ajax({
                url: '<?= base_url('admin/'.SAM_MODULE.'/itemsSuggestions') ?>',
                dataType: "json",
                data: {
                    term: request.term,
                    for: forItem,
    ...
    function addItem(data, manual = false) {
        ...
        $.ajax({
            type: 'post',
            url: "<?= admin_url('sales_marketing/add_insert_items/') ?>" + deal_id,
            dataType: "json",
            data: {
                item_id: item_id,
            },
    ...
    $("body").on('click', '.itemManualy', function (e) {
        ...
        $.ajax({
            type: 'post',
            url: '<?= base_url() ?>admin/sales_marketing/itemAddedManualy/',
            data: data,
            dataType: "json",
            success: function (data) {
                if (data !== null) {
