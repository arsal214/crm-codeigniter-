...
<div class=" inline-block full-width simple-editor">
    <?php echo form_open_multipart(admin_url(SAM_MODULE.'/add_deals_comment'), ['id' => 'deal_comment_form', 'class' => 'dropzone dropzone-manual', 'style' => 'min-height:auto;background-color:#fff;']); ?>
        ...
        foreach ($deals_details->comments as $comment) {
            ...
            if ($comment['file_id'] != 0) {
                ...
                foreach ($comment['attachments'] as $attachment) {
                    ...
                    $is_image = false;
                    $path = get_upload_path_for_sam() . $deals_details->id . '/' . $attachment['file_name'];
                    $href_url = site_url('admin/'.SAM_MODULE.'/file_download/deals_comments/' . $attachment['attachment_key']);
                ...
                $comment['content'] .= '<div class="text-center download-all">
                   <hr class="hr-10" />
                   <a href="' . admin_url(SAM_MODULE.'/download_files/' . $deals_details->id . '/' . $comment['id']) . '" class="bold">' . _l('download_all') . ' (.zip)
                   </a>
                   </div>';
            }
...
<script type="text/javascript">
    ...
    function add_deals_comment(deal_id) {
        ...
        $.post(admin_url + "sales_marketing/add_deals_comment", data).done(function (response) {
    ...
    function remove_deal_comment(commentid) {
        if (confirm_delete()) {
            requestGetJSON("sales_marketing/remove_comment/" + commentid).done(function (
                response
            ) {
    ...
    function save_edited_comment(id, deal_id) {
        ...
        $.post(admin_url + "sales_marketing/edit_comment", data).done(function (response) {
