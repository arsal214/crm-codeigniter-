...
<table class="table table-hover pt-3" id="">
    ...
    <tbody>
    <?php
    $all_task_info = sam_join_data(db_prefix() . 'tasks', db_prefix() . 'tasks.*,tbl_sam.title,(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'tasks.id and rel_type="task" ORDER by tag_order ASC) as tags', array(db_prefix() . 'tasks.rel_type' => 'sam', db_prefix() . 'tasks.rel_id' => $deals_details->id,), ['tbl_sam' => 'tbl_sam.id = ' . db_prefix() . 'tasks.rel_id AND ' . db_prefix() . 'tasks.rel_type = "sam"'], 'object');
    ...
    if (!empty($all_task_info)) {
        ...
        foreach ($all_task_info as $key => $v_task) {
            ...
            if (!empty($v_task->rel_id)) {
                $link = admin_url(SAM_MODULE.'/details/' . $v_task->rel_id);
