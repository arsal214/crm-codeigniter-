...
if ($task_timer_id) {
    $sub_active = 2;
    $deals_email_details = get_sam_row('tbl_sam_email', array('id' => $task_timer_id));
    $username_email_details = get_sam_row('tblstaff', array('staffid' => $deals_email_details->user_id));
}
    ...
    <ul class="nav nav-tabs" style="margin-top: -20px; margin-bottom: 0px">
        ...
        <?php if (!empty($edited)) { ?>
            <li class="<?= $sub_active == 2 ? 'active' : ''; ?>"><a href="#new_email"
                                                                    data-toggle="tab"><?= _l('sam_new_email') ?></a>
            </li>
        <?php } ?>
    </ul>
    <div class="tab-content bg-white">
        ...
        <div class="tab-pane <?= $sub_active == 1 ? 'active' : ''; ?>" id="all_email">
            <div class="table-responsive">
                <table class="table table-striped " cellspacing="0" width="100%">
                    ...
                    <tbody>
                    <?php
                    $all_deals_email = get_sam_result('tbl_sam_email', array('deals_id' => $deals_details->id));
                    ...
                    if (!empty($all_deals_email)) :
                        foreach ($all_deals_email as $v_emails) :
                            ...
                            <tr id="table-meeting-<?= $v_emails->id ?>">
                                ...
                                <td>
                                    <a href="<?= base_url('admin/'.SAM_MODULE.'/email_details/' . $v_emails->id) ?>"
                                       class="btn btn-xs btn-info" data-placement="top" data-toggle="modal"
                                       data-target="#myModal">
                                        <i class="fa fa-list "></i></a>
                                    <a href="<?= base_url('admin/' . SAM_MODULE . '/details/' . $deals_details->id . '/email/' . $v_emails->id) ?>" 
                                       class="btn btn-xs btn-warning">
                                       <i class="fa fa-edit"></i> Edit
                                    </a>
                                    <!-- Delete Button -->
                                    <a href="<?= base_url('admin/' . SAM_MODULE . '/delete_deals_email/' . $deals_details->id . '/' . $v_emails->id) ?>" 
        ...
        <div class="tab-pane <?= $sub_active == 2 ? 'active' : ''; ?>" id="new_email">
            <div class="panel-body">
                ...
                <?php echo form_open(base_url('admin/'.SAM_MODULE.'/send_mail/' . $id), array('id' => 'deals_email_form', 'enctype' => 'multipart/form-data', 'data-parsley-validate' => '', 'role' => 'form')); ?>
