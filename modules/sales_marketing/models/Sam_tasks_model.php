...
class Sam_tasks_model extends App_Model
{
    ...
    public function get_timers($task_id, $where = [])
    {
        ...
        return $this->db->get(db_prefix() . '_sam_taskstimers')->result_array();
    }
    ...
    public function get_task_timer($where)
    {
        ...
        return $this->db->get(db_prefix() . '_sam_taskstimers')->row();
    }
    ...
    public function is_timer_started_for_task($id, $where = [])
    {
        ...
        $this->db->where('end_time IS NULL');
        $this->db->where($where);
        $results = $this->db->count_all_results(db_prefix() . '_sam_taskstimers');
    ...
    public function get_last_timer($task_id, $staff_id = '')
    {
        ...
        $this->db->limit(1);
        $timer = $this->db->get(db_prefix() . '_sam_taskstimers')->row();
    ...
    public function get($id, $where = [])
    {
        ...
        $this->db->where('id', $id);
        $this->db->where($where);
        $task = $this->db->get(db_prefix() . '_sam_tasks')->row();
        if ($task) {
    ...
    public function add_timesheet($data=array())
    {
        if (isset($data['timesheet_duration']) && $data['timesheet_duration'] != '' && $data['timesheet_duration'] != ':') {
            ...
            $start_time = to_sql_date($data['timesheet_start_time'], true);
            $end_time   = to_sql_date($data['timesheet_end_time'], true);
            $start_time = strtotime($start_time);
            $end_time   = strtotime($end_time);
            $time_spent = $end_time - $start_time;
        }

        $this->db->insert(db_prefix() . '_sam_taskstimers', [
            'sam_id'                => $data['sam_id'],
            'pipeline_id'           => $data['pipeline_id'],
            'stage_id'              => $data['stage_id'],
            'start_time'            => $start_time,
            'end_time'              => $end_time,
            'timesheet_duration'    => $data['timesheet_duration'],
     ...
     * @return boolean
     */
    public function timer_tracking($data=array(),$action=null)
    { 
        if($action=='insert'){
        ...
        elseif($action=='update'){
            $end_time = time();
            $time_spent = $end_time - $data['start_time'];
            $this->db->where('end_time IS NULL');
            $this->db->where('is_timer', 1);
            $this->db->where('sam_id', $data['sam_id']);
            $this->db->where('pipeline_id', $data['pipeline_id']);
            $this->db->where('stage_id', $data['stage_id']);
            $this->db->where('staff_id', get_staff_user_id());
            $this->db->update(db_prefix() . '_sam_taskstimers', [
                'end_time'      => $end_time,
                'time_spent'    => $time_spent,
    ...
    public function get_timers2($where = [])
    {
        $this->db->where($where);
    ...
    public function is_timer_started_for_task2($where = [])
    {
        $this->db->where('end_time IS NULL');
        $this->db->where('deleted',0);
        $this->db->where($where);
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . '_sam_taskstimers',['deleted' => 1]);
        if ($this->db->affected_rows() > 0) {
    ...
    public function timesheet($data)
    {
        ...
        if (!isset($data['timer_id']) || (isset($data['timer_id']) && $data['timer_id'] == '')) {
            ...
            $this->db->where('sam_id', $date['sam_id']);
            $this->db->where('pipeline_id', $date['pipeline_id']);
            $this->db->where('stage_id', $date['stage_id']);
    ...
    public function getTotalSpentTimeOnDeal($sam_id=0){
        
        $where = "where t.sam_id = ".$sam_id." AND t.deleted=0";
        $query = '
                SELECT t.staff_id, t.pipeline_id, sum(t.time_spent) as total_time_spent 
