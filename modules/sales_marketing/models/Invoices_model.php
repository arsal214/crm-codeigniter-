...
class Invoices_model extends App_Model
{
        ...
        if ($insert_id) {
            ...
            if (isset($data['clientid']) && !empty($data['clientid'])) {
                ...
                if ($clientQuery->num_rows() > 0) {
                    $clientRow = $clientQuery->row();
                    $company = $clientRow->company;
                }
            }
            $description = !empty($items[1]['description']) ? $items[1]['description'] : '';
           $sam_id = $data['sam_id'];
           $status = self::STATUS_UNPAID;
            if ($company && $description) {
                $response = $this->insertEventPackageClient($company, $description, $status);
                if (is_numeric($response)) {
                    echo '<div style="padding: 15px; background-color: #dff0d8; color: #3c763d; border: 1px solid #d6e9c6; border-radius: 5px; font-size: 16px; text-align: center;">
                            🎉 Congratulations! The event package client has been added successfully In UNIRANKS.
                          </div>';
                    echo '<script>
                            setTimeout(function() {
                                window.location.href = "https://crm.mazajnet.com/admin/sales_marketing/details/' . $sam_id . '/invoices";
                            }, 2500);
                          </script>';
                } else {
                    echo '<div style="padding: 15px; background-color: #f2dede; color: #a94442; border: 1px solid #ebccd1; border-radius: 5px; font-size: 16px; text-align: center;">
                            ❌ Oops! Something went wrong  While Saving event package client In UNIRANKS: ' . htmlspecialchars($response) . '
                          </div>';
                    echo '<script>
                            setTimeout(function() {
                                window.location.href = "https://crm.mazajnet.com/admin/sales_marketing/details/' . $sam_id . '/invoices";
                ...
                echo '<div style="padding: 15px; background-color: #fcf8e3; color: #8a6d3b; border: 1px solid #faebcc; border-radius: 5px; font-size: 16px; text-align: center;">
                        ⚠ Please provide all the required information: ' . (!$company ? '<b>Company</b>' : '') . (!$company && !$description ? ' and ' : '') . (!$description ? '<b>Description</b>' : '') . '.
                      </div>';
                echo '<script>
                        setTimeout(function() {
                            window.location.href = "https://crm.mazajnet.com/admin/sales_marketing/details/' . $sam_id . '/invoices";
   ...
   public function insertEventPackageClient($company, $description, $status)
    {
        $uniranks_db                            = $this->load->database('uniranks_new', TRUE);
        $uniranks_db->select('id');
        $uniranks_db->where('title', $description);
        $event_package                          = $uniranks_db->get('event_packages')->row();
    
        if (!$event_package)
        {
            return 'Event package not available';
        }
    
        $event_package_id                       = $event_package->id;
    
        $uniranks_db->where('id', $event_package_id);
        $package_data                           = $uniranks_db->get('event_packages')->row_array();
    
        if (!$package_data)
        {
            return 'Event package data not found';
        }
    
        $uniranks_db->select('id, event_credit, ur_credit');
        $uniranks_db->where('university_name', $company);
        $university                             = $uniranks_db->get('universities')->row();
        ...
        [
            ...
            'is_top_up'                             => 0,
            'university_fair_schools_campus'        => $package_data['university_fair_schools_campus'],
            'international_virtual_events'          => $package_data['international_virtual_events'],
            'career_talks_school_campus'            => $package_data['career_talks_school_campus'],
            'work_shops_university_campus'          => $package_data['work_shops_university_campus'],
            'open_days_university_campus'           => $package_data['open_days_university_campus'],
            'student_for_a_day_university_campus'   => $package_data['student_for_a_day_university_campus'],
            'competition_university_campus'         => $package_data['competition_university_campus'],
            'repositories_credit'                   => $package_data['repositories_credit'],
            'schools_tours'                         => $package_data['schools_tours'],
            'countries'                             => $package_data['countries'],
            'status'                                => $status,
            'start_date'                            => date('Y-m-d'),
            'expire_date'                           => date('Y-m-d', strtotime('+' . $package_data['expire_in_days'] . ' days')),
            'created_by_id'                         => '',
            'created_at'                            => date('Y-m-d H:i:s'),
            'updated_at'                            => date('Y-m-d H:i:s'),
        ];
    
        $insert_success                             = $uniranks_db->insert('event_package_clients', $insert_data);
    
        if (!$insert_success) {
            log_message('error', 'Insert Failed: ' . $uniranks_db->last_query());
            return 'Insert operation failed';
        }
    
        $event_package_client_id = $uniranks_db->insert_id();
    
        $credit_in_value                            = $package_data['university_fair_schools_campus'] + 
                                                      $package_data['international_virtual_events'] + 
                                                      $package_data['career_talks_school_campus'] + 
                                                      $package_data['work_shops_university_campus'] + 
                                                      $package_data['open_days_university_campus'] + 
                                                      $package_data['student_for_a_day_university_campus'] + 
                                                      $package_data['competition_university_campus'];
                                                      
        $ur_credit_in                               = $package_data['repositories_credit'];
       
        $updated_event_credit                       = $university->event_credit + $credit_in_value;
        
        $updated_ur_credit                          = $university->ur_credit + $ur_credit_in;
    
        $uniranks_db->where('id', $university_id);
        $uniranks_db->update('universities',[
            'event_credit'                          => $updated_event_credit,
            'ur_credit'                             => $updated_ur_credit
        ]);

    
        $event_name                                 = $package_data['title'];
        ...
        [
            ...
            'credit_out'                            => '',
            'credit_in'                             => $credit_in_value,
            'by_user_id'                            => '',
            'created_at'                            => date('Y-m-d H:i:s'),
            'updated_at'                            => date('Y-m-d H:i:s'),
            'event_package_client_id'               => $event_package_client_id,
            'event_type'                            => '',
            'event_accepted_method'                 => '',
        ];
    
        $transaction_insert_success                 = $uniranks_db->insert('event_credit_transactions', $transaction_data);
    
        if (!$transaction_insert_success)
        {
            log_message('error', 'Transaction Insert Failed: ' . $uniranks_db->last_query());
            return 'Transaction insert failed';
        }
        ...
        [
            'agent_id'                              => '',
            'university_id'                         => $university_id,
            'description'                           => $package_data['title'],
            'avg_ur_cost'                           => '',
            'quantity_purchased'                    => '0.00',
            'ur_credit_out'                         => '0.00',
            'ur_credit_in'                          => $ur_credit_in,
            'by_user_id'                            => '',
            'created_at'                            => date('Y-m-d H:i:s'),
            'updated_at'                            => date('Y-m-d H:i:s'),
            'event_package_client_id'               => $event_package_client_id,
        ...
        $repository_insert_success                  = $uniranks_db->insert('repository_transactions', $repository_data);
    
        if (!$repository_insert_success)
        {
            log_message('error', 'Repository Transaction Insert Failed: ' . $uniranks_db->last_query());
            return 'Repository transaction insert failed';
        }
    
        return $event_package_client_id;
    }
    ...
    public function copy($id)
    {
        ...
        $new_invoice_data             = [];
        $new_invoice_data['sam_id']   = $_invoice->sam_id;
        $new_invoice_data['pro_id']   = $_invoice->pro_id;
        $new_invoice_data['contract_id'] = $_invoice->contract_id;
