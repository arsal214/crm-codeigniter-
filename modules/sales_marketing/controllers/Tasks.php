...
class Tasks extends AdminController
{
    public function __construct()
    {
        ...
        $this->load->model('sam_tasks_model');
    }
    ...
    public function list_tasks($id = '')
    {
        ...
        $this->load->view('sales_marketing/tasks/manage', $data);
    }
    ...
    public function ajax_search_assign_task_to_timer()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->db->from(db_prefix() . '_sam_tasks');
            $this->db->where('' . db_prefix() . '_sam_tasks.id IN (SELECT taskid FROM ' . db_prefix() . '_sam_task_assigned WHERE staffid = ' . get_staff_user_id() . ')');
    ...
    public function task($id = '')
    {
        ...
        $data['id']      = $id;
        $data['title']   = $title;
        $this->load->view('sales_marketing/tasks/task', $data);
    }
    ...
    public function get_staff_started_timers($return = false)
    {
        ...
        $_data['html']         = $this->load->view('sales_marketing/tasks/started_timers', $data, true);
     ...
     * @return mixed
     */
    public function get_task_data($sam_id=null,$pipeline_id=null,$stage_id=null)
    {
        $cond = array(
            'sam_id'        => $sam_id,
        ...
        $timers = $this->sam_tasks_model->get_timers2($cond);
        ...
        $this->load->view('sales_marketing/tasks/task_view_template', $data);
    }
    
    public function add_timesheet($sam_id=null,$pipeline_id=null,$stage_id=null)
    {         
        $data = $this->input->post();
        ...
        $data['pipeline_id'] = $pipeline_id;
        $data['stage_id'] = $stage_id;
        $success = $this->sam_tasks_model->add_timesheet($data);
        if ($success === true) {
            $this->session->set_flashdata('task_single_timesheets_open', true);
            $type = true;
            $message = _l('added_successfully', _l('project_timesheet'));
        } else {
            $message = _l('failed_to_add_timesheet');
            $type = false;
        }
    ...
    public function timer_tracking($sam_id=null,$pipeline=null,$stage_id=null)
    {    
        $data['sam_id']       = $sam_id;
        $data['pipeline_id']  = $pipeline;
        $data['stage_id']     = $stage_id; 
        //check if timer start then stop it and update the record
        $timer_record = get_one_timer($sam_id,$pipeline,$stage_id);
        if($timer_record){
            $data['start_time'] = $timer_record->start_time;
            $this->sam_tasks_model->timer_tracking($data,'update');  
            $msg = 'The Timer is stopped';  
        }
        //insert new timer track into db
        else{
            $this->sam_tasks_model->timer_tracking($data,'insert');
            $msg = 'The Timer is started';    
        }
    ...
    public function delete_timesheet($id)
    {
        if (1) {
            $alert_type = 'warning';
            $success    = $this->sam_tasks_model->delete_timesheet($id);
            if ($success) {
                //$this->session->set_flashdata('task_single_timesheets_open', true);
                $message = _l('deleted', 'Timesheet Deleted');
                $success = true;
            }
            else{
                $message = _l('deleted', 'Timesheet Deletion Failed');
                $success = false;
            }
            if (!$this->input->is_ajax_request()) {   
                //redirect(previous_url() ?: $_SERVER['HTTP_REFERER']);
    ...
    public function update_timesheet($sam_id=null,$pipeline_id=null,$stage_id=null)
    {
        if ($this->input->is_ajax_request()) {
            if (1) {
                $data = $this->input->post();
                ...
                $data['pipeline_id'] = $pipeline_id;
                $data['stage_id'] = $stage_id;
                $success = $this->sam_tasks_model->timesheet($data);
                if ($success === true) {
                    //$this->session->set_flashdata('task_single_timesheets_open', true);
