...
class Invoices extends AdminController
{
    ...
    public function list_invoices($id = '')
    {
        ...
        $this->load->view(SAM_MODULE.'/invoices/manage', $data);
    }

    /* Add new invoice or update existing */
    public function invoice($id = '', $sam_id = '')
    {
         $cond = array('id' => $sam_id);
        $sam_rec = $this->sam_model->getAllRecords("tbl_sam","*",$cond);
                    //echo "<pre>"; print_r($sam_rec); exit;
        ...
        if ($this->input->post()) {
            ...
            if ($id == '' || $id == 0) {
                ...
                $invoice_data['sam_id'] = $sam_id;
                $id = $this->invoices_model->add($invoice_data);
                if ($id) {
                    add_activity_transactions($sam_id,'added new invoice');
                    set_alert('success', _l('added_successfully', _l('invoice')));
                    $redUrl = admin_url(SAM_MODULE.'/details/'.$sam_id.'/invoices');
        ...
        if ($id == '' || $id == 0) {
            ...
            $data['billable_tasks'] = [];
            $data['invoices_to_merge'] = []; // Empty array define kar dein
            $data['expenses_to_bill'] = []; // Empty array define kar dein
        } else {
        ...
        $sam_rec = $this->sam_model->getAllRecords("tbl_sam",'*',array('id'=>$sam_id));
        //echo "<pre>"; print_r($sam_rec); exit;
        $data['sam_rec'] = $sam_rec;
        $this->load->view(SAM_MODULE.'/invoices/invoice', $data);
    }
    ...
    public function recurring($id = '')
    {
        ...
        $data['invoices_years']       = $this->invoices_model->get_invoices_years();
        $data['invoices_sale_agents'] = $this->invoices_model->get_sale_agents();
        $this->load->view(SAM_MODULE.'/invoices/recurring/list', $data);
    }
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            $data['merge_info'] = $this->load->view(SAM_MODULE.'/invoices/merge_invoice', $_data, true);
            ...
            $data['expenses_bill_info'] = $this->load->view(SAM_MODULE.'/invoices/bill_expenses', $__data, true);
    ...
    public function pause_overdue_reminders($id)
    {
        ...
        redirect(admin_url(SAM_MODULE.'/invoices/#' . $id));
    }
    ...
    {
        if (!$id) {
            redirect(admin_url(SAM_MODULE.'/invoices'));
        }
        if (staff_cant('create', 'invoices')) {
            access_denied('invoices');
        }
        ...
        if ($new_id) {
            ...
            redirect(admin_url(SAM_MODULE.'/invoices/invoice/' . $new_id.'/'.$sam_id));
        } else {
            set_alert('success', _l('invoice_copy_fail'));
        }
        redirect(admin_url(SAM_MODULE.'/invoices/invoice/' . $id.'/'.$sam_id));
    }
    ...
    public function get_invoice_data_ajax($id)
    {
        ...
        $this->load->view(SAM_MODULE.'/invoices/invoice_preview_template', $data);
    }
    ...
    public function record_invoice_payment_ajax($id)
    {
        ...
        $data['payments'] = $this->payments_model->get_invoice_payments($id);
        $this->load->view(SAM_MODULE.'/invoices/record_payment_template', $data);
    }
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id) {
                ...
                redirect(admin_url(SAM_MODULE.'/payments/payment/' . $id));
            } else {
                set_alert('danger', _l('invoice_payment_record_failed'));
            }
            redirect(admin_url(SAM_MODULE.'/invoices/#' . $this->input->post('invoiceid')));
        }
    }
    ...
    public function delete_payment($id, $invoiceid)
    {
        ...
        $this->load->model('payments_model');
        if (!$id) {
            redirect(admin_url(SAM_MODULE.'/payments'));
        }
        ...
        redirect(admin_url(SAM_MODULE.'/invoices/#' . $invoiceid));
    }
    ...
    {
        if (!$id) {
            redirect(admin_url(SAM_MODULE.'/invoices/list_invoices'));
        }
