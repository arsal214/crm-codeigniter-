...
class Si_lead_filters extends AdminController 
{
	...
	public function leads_filter()
	{
		...
		//get query Leads
		$sqlLeadsSelect = db_prefix().'leads.*,CONCAT('.db_prefix().'staff.firstname," ",'.db_prefix().'staff.lastname) as staff_name,'.db_prefix().'leads_sources.name as source_name,(SELECT count(leadid) FROM ' . db_prefix() . 'clients WHERE ' . db_prefix() . 'clients.leadid=' . db_prefix() . 'leads.id) as is_converted';
    ...
    private function get_leads_school_summary()
    {
        ...
        //get query Leads
        $sqlLeadsSelect = db_prefix().'leads_school.status,count(*) as total,sum(lost) as total_lost,sum(junk) as total_junk';
        $this->db->select($sqlLeadsSelect);
        ...
        if ($tag && !in_array('',$tag)) {
            $this->db->join(db_prefix() . 'taggables' , '('.db_prefix() . 'taggables.rel_id = ' . db_prefix() . 'leads_school_school.id and rel_type=\'lead\')','left');
            $this->db->where_in('tag_id', $tag);
            $this->db->group_by(db_prefix() . 'leads_school.id');
        }
        if ($country && !in_array('',$country)) {
            if(in_array(-1,$country))//if country is unknown
                $country[]=0;
        ...
        $this->db->group_by('status');
        $result = $this->db->get(db_prefix() . 'leads_school');
        $status_count = array();
        $total_leads = 0;
        if($result) {
            $result = $result->result_array();
