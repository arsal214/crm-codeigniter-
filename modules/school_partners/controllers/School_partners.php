...
class School_partners extends AdminController
{
    public function __construct()
    {
        parent::__construct();		
        generate_partners_tables();
    ...
    public function index()
    {
        ...
        $this->load->model('school_partners_model');
        $data['contract_types'] = $this->contracts_model->get_contract_types();
        $data['groups']         = $this->school_partners_model->get_groups();
        ...
        $data['customer_admins'] = $this->school_partners_model->get_customers_admin_unique_ids();
        ...
        $data['contacts_logged_in_today'] = $this->school_partners_model->get_contacts('', 'last_login LIKE "' . date('Y-m-d') . '%"' . $whereContactsLoggedIn);

        $data['countries'] = $this->school_partners_model->get_clients_distinct_countries();
    ...
    public function table()
    {
        ...
        get_partners_table_data('school_partners');
    }
    ...
    public function all_contacts()
    {
        ...
        $this->load->view('clients/all_contacts', $data);
    }
    ...
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                $id = $this->school_partners_model->add($data);
                if (!has_permission('customers', '', 'view')) {
                    $assign['customer_admins']   = [];
                    $assign['customer_admins'][] = get_staff_user_id();
                    $this->school_partners_model->assign_admins($assign, $id);
                }
                if ($id) {
                    // if(isset($_GET['type'])){
                    //     set_alert('success', _l('added_successfully', _l('client')));
                    //     if ($save_and_add_contact == false) {
                    //         redirect(admin_url('clients/client/' . $id.'?type=sp'));
                    //     } else {
                    //         redirect(admin_url('clients/client/' . $id . '?group=contacts&new_contact=true&type=sp'));
                    //     }
                    // }else{
                        set_alert('success', _l('added_successfully', _l('client')));
                        if ($save_and_add_contact == false) {
                            redirect(admin_url('school_partners/client/' . $id));
                        } else {
                            redirect(admin_url('school_partners/client/' . $id . '?group=contacts&new_contact=true'));
                        }
                    // }
                }
            } else {
                ...
                //         }
                //     }
                //     $success = $this->school_partners_model->update($this->input->post(), $id);
                //     if ($success == true) {
                //         set_alert('success', _l('updated_successfully', 'Schools Partner'));
                //     }
                //     redirect(admin_url('school_partners/client/' . $id.''));
                // }else{
                    if (!has_permission('customers', '', 'edit')) {
        ...
        if ($group != 'contacts' && $contact_id = $this->input->get('contactid')) {
            redirect(admin_url('school_partners/client/' . $id . '?group=contacts&contactid=' . $contact_id));
        }
...
//        exit();
        if ($id == '') {
            ...
            $client                = $this->school_partners_model->get($id);
            ...
            $data['contacts'] = $this->school_partners_model->get_contacts($id);
            ...
            if ($group == 'profile') {
                $data['customer_groups'] = $this->school_partners_model->get_customer_groups($id);
                $data['customer_admins'] = $this->school_partners_model->get_admins($id);
            } elseif ($group == 'attachments') {
                $data['attachments'] = get_all_customer_attachments($id);
            } elseif ($group == 'vault') {
                $data['vault_entries'] = hooks()->apply_filters('check_vault_entries_visibility', $this->school_partners_model->get_vault_entries($id));
            ...
            //         if (is_empty_customer_company($data['client']->userid,'school_partners')) {
            //             $data['client']->company = '';
            //         }
            //     }
            // }else{
                if (!empty($data['client']->company)) {
                    // Check if is realy empty client company so we can set this field to empty
                    // The query where fetch the client auto populate firstname and lastname if company is empty
                    if (is_empty_parnters_company($data['client']->userid)) {
                        $data['client']->company = '';
                    }
                }
            // }
    ...
    public function form_contact($customer_id, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
                ...
                $id      = $this->school_partners_model->add_contact($data, $customer_id);
                $message = '';
                $success = false;
                if ($id) {
            ...
            $original_contact = $this->school_partners_model->get_contact($contact_id);
            $success          = $this->school_partners_model->update_contact($data, $contact_id);
            $message          = '';
            ...
            if ($updated == true) {
                $contact = $this->school_partners_model->get_contact($contact_id);
        ...
        if ($contact_id == '') {
            ...
            $data['contact'] = $this->school_partners_model->get_contact($contact_id);
    ...
    public function confirm_registration($client_id)
    {
        ...
        $this->school_partners_model->confirm_registration($client_id);
    ...
    {
        $this->school_partners_model->delete_contact_profile_image($contact_id);
    }
    ...
    public function assign_admins($id)
    {
        ...
        $success = $this->school_partners_model->assign_admins($this->input->post(), $id);
        if ($success == true) {
            set_alert('success', _l('updated_successfully', _l('client')));
        }
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        $contact      = $this->school_partners_model->get_contact($id);
        $hasProposals = false;
        if ($contact && is_gdpr()) {
        ...
        $this->school_partners_model->delete_contact($id);
        if ($hasProposals) {
    ...
    public function delete_attachment($customer_id, $id)
    {
        if (has_permission('customers', '', 'delete') || is_customer_admin($customer_id)) {
            $this->school_partners_model->delete_attachment($id);
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('school_partners'));
        }

            $response = $this->school_partners_model->delete_school_partner($id);

        if (is_array($response) && isset($response['referenced'])) {
            set_alert('warning', _l('customer_delete_transactions_warning', _l('invoices') . ', ' . _l('estimates') . ', ' . _l('credit_notes')));
        } elseif ($response == true) {
            set_alert('success', _l('deleted', "Schools Partner"));
        } else {
    ...
    {
        echo json_encode($this->school_partners_model->get_customer_billing_and_shipping_details($id));
    }
    ...
    public function change_contact_status($id, $status)
    {
        if (has_permission('customers', '', 'edit') || is_customer_admin(get_user_id_by_contact_id($id))) {
            if ($this->input->is_ajax_request()) {
                $this->school_partners_model->change_contact_status($id, $status);
            }
        }
    }
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->school_partners_model->change_client_status($id, $status);
        }
    }
    ...
    public function groups()
    {
        ...
        if ($this->input->is_ajax_request()) {
            get_partner_table_data('school_partners_groups');
        }
        $data['title'] = _l('customer_groups');
        $this->load->view('clients/groups_manage', $data);
    }
    ...
    public function group()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($data['id'] == '') {
                $id      = $this->school_partners_model->add_group($data);
                ...
                $success = $this->school_partners_model->edit_group($data);
                $message = '';
                if ($success == true) {
    ...
    public function delete_group($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('school_partners/groups'));
        }
        $response = $this->school_partners_model->delete_group($id);
        if ($response == true) {
        ...
        // if(isset($_GET["gtype"])){
        //     redirect(admin_url('clients/school_partner_group')."?gtype=spg");
        // }else{
            redirect(admin_url('school_partners/groups'));
        // }
    }
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        if($_POST['type'] == 'sp'){
                            if ($this->school_partners_model->delete_school_partner($id)) {
                                $total_deleted++;
                            }
                        }else{
                            if ($this->school_partners_model->delete($id)) {
                                $total_deleted++;
                            }
                        }
                    } else {
    ...
    public function vault_entry_create($customer_id)
    {
        ...
        $this->school_partners_model->vault_entry_create($data, $customer_id);
    ...
    {
        $entry = $this->school_partners_model->get_vault_entry($entry_id);
        ...
        if ($entry->creator == get_staff_user_id() || is_admin()) {
            ...
            $this->school_partners_model->vault_entry_update($entry_id, $data);
    ...
    {
        $entry = $this->school_partners_model->get_vault_entry($id);
        if ($entry->creator == get_staff_user_id() || is_admin()) {
            $this->school_partners_model->vault_entry_delete($id);
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function vault_encrypt_password()
    {
        ...
        $vault    = $this->school_partners_model->get_vault_entry($id);
    ...
    public function statement_pdf()
    {
        ...
        $data['statement'] = $this->school_partners_model->get_statement($customer_id, to_sql_date($from), to_sql_date($to));
    ...
    public function send_statement()
    {
        ...
        $success = $this->school_partners_model->send_statement_to_email($customer_id, $send_to, $from, $to, $cc);
