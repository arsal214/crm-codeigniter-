...
class School_partners_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        //     $table = 'clients';
        // }
        $this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . $table)) . ',' . get_sql_select_client_company('company',$table));

        $this->db->join(db_prefix() . 'countries', '' . db_prefix() . 'countries.country_id = ' . db_prefix() . $table.'.country', 'left');
        $this->db->join(db_prefix() . 'contacts', '' . db_prefix() . 'contacts.userid = ' . db_prefix() . $table.'.userid AND is_primary = 1', 'left');
        ...
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . $table.'.userid', $id);
            $client = $this->db->get(db_prefix() . $table)->row();
    ...
    public function add($data, $withContact = false)
    {
        ...
        if(isset($_GET['type'])){
            $this->db->insert(db_prefix() . 'school_partners', array_merge($data, [
    ...
    public function update($data, $id, $client_request = false)
    {
        ...
        // if(isset($_GET['type'])){
            $this->db->update(db_prefix() . 'school_partners', $data);
        // }else{
    ...
    public function delete_school_partner($id)
    {
        $affectedRows = 0;
        ...
        $this->db->where('userid', $id);
        $this->db->delete(db_prefix() . 'school_partners');
        if ($this->db->affected_rows() > 0) {
            $affectedRows++;
            // Delete all user contacts
            $this->db->where('userid', $id);
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix().'school_partners_groups');
        if ($this->db->affected_rows() > 0) {
            // $this->db->where('groupid', $id);
            // $this->db->delete(db_prefix().'school_partners_groups');
            // hooks()->do_action('customer_group_deleted', $id);
            log_activity('Partners Group Deleted [ID:' . $id . ']');
            return true;
        }
        return false;
    }
    ...
    {
        $this->db->insert(db_prefix().'school_partners_groups', $data);
    ...
    public function edit_group($data)
    {
        ...
        // }
        // else{
            $this->db->update(db_prefix().'school_partners_groups', [
                'name' => $data['name'],
            ]);
        // }
        if ($this->db->affected_rows() > 0) {
            log_activity('Partners Group Updated [ID:' . $data['id'] . ']');
