...
$sIndexColumn = 'id';
$sTable       = $this->ci->uri->segment(4) != 'sl' ? db_prefix() . 'leads' : db_prefix() . 'leads_school';
...
if ($this->ci->input->post('custom_view')) {
    ...
    if ($filter == 'lost') {
        ...
        array_push($where, 'AND ' .$sTable.'.id IN (SELECT lead_id FROM ' . db_prefix() . 'consents WHERE purpose_id=' . $this->ci->db->escape_str(strafter($filter, 'consent_')) . ' and action="opt-in" AND date IN (SELECT MAX(date) FROM ' . db_prefix() . 'consents WHERE purpose_id=' . $this->ci->db->escape_str(strafter($filter, 'consent_')) . ' AND lead_id=' .$sTable.'.id))');
    }
}
...
foreach ($rResult as $aRow) {
    ...
    $hrefAttr = 'href="' . admin_url('leads/index/' . $aRow['id']) .'/'. ($this->ci->uri->segment(4) != 'sl' ? '' : '?type=sl'). '" onclick="init_lead(' . $aRow['id'] . ');return false;"';
    ...
    if (!$locked) {
        $nameRow .= ' | <a href="' . admin_url('leads/index/' . $aRow['id'] . '?edit=true') .'/'. (($this->ci->uri->segment(4) == 'sl') ? '?type=sl' : ''). '" onclick="init_lead(' . $aRow['id'] . ', true);return false;">' . _l('edit') . '</a>';
    }

    if ($aRow['addedfrom'] == get_staff_user_id() || $has_permission_delete) {
        $nameRow .= ' | <a href="' . admin_url('leads/delete/' . $aRow['id']) .'/'. (($this->ci->uri->segment(4) == 'sl') ? '?type=sl' : ''). '" class="_delete text-danger">' . _l('delete') . '</a>';
    }
    $nameRow .= '</div>';
